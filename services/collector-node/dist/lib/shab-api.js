...
import { ShabApiError } from '../types/shab.js';
export class ShabApiService {
    ...
    async fetchPublicationXml(metaId) {
        ...
        if (!response.ok) {
            throw new ShabApiError(`SHAB XML request failed: ${response.status} ${response.statusText}`, response.status);
        }
    ...
    async makeRequest(url, attempt = 1) {
        try {
            ...
            if (!response.ok) {
                throw new ShabApiError(`SHAB API request failed: ${response.status} ${response.statusText}`, response.status, await response.text());
            }
            ...
            throw error instanceof ShabApiError ? error : new ShabApiError(`SHAB API request failed after ${this.maxRetries} attempts: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
